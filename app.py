...
# Script para bloquear o Google Tradutor (Evita o erro removeChild)
...
st.caption(f"🛡️ Painel de Contingência Ativado • {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
# --- SISTEMA DE NOTÍCIAS COM CIRCUITO DE SEGURANÇA ---
st.subheader("📰 Notícias em Tempo Real (Fontes Oficiais)")

def buscar_noticias_seguro():
    fontes = {
...
noticias = buscar_noticias_seguro()
        ...
        with target_col:
            ...
            <div class="news-card">
                ...
                <div style='margin-top: 12px;'>
                    <a href="{n['link']}" target="_blank" style="color: #00FF88; text-decoration: none; font-size: 14px;">LER NOTÍCIA →</a>
                </div>
            </div>
            """, unsafe_allow_html=True)
else:
    # PAINEL DE EMERGÊNCIA (Caso o servidor de notícias falhe)
    st.warning("⚠️ O servidor de sincronização está instável. Use os links de acesso direto abaixo:")
    e1, e2, e3 = st.columns(3)
    e1.link_button("🌐 Abrir InfoMoney", "https://www.infomoney.com.br")
    e2.link_button("🌐 Abrir G1 Economia", "https://g1.globo.com")
    e3.link_button("🌐 Abrir Valor Econômico", "https://valor.globo.com")

st.markdown("---")
st.caption("🔒 Painel blindado contra desinformação via Protocolos RSS Diretos.")
