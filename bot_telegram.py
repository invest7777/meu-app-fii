...
# Valores CONFERIDOS da sua imagem (Soma exata: R$ 29.773,28)
carteira_valores = {
    "MXRF11": 2001.24,
    "RECR11": 2090.66,
    "VGHF11": 3009.60,
...
def enviar_relatorio_vip():
    try:
        ...
        # Estimativa de dividendos (Média de 0,95% ao mês)
        dividendos_estimados = total * 0.0095 
        data_hora = datetime.now().strftime('%d/%m/%Y %H:%M')
        
        # 1. GERAR GRÁFICO (Modo Escuro Premium)
        ...
        plt.title(f"📊 ALOCAÇÃO DE ATIVOS\nInvestment Bank", color='white', fontsize=16, pad=20)
        ...
        # 2. MONTAR MENSAGEM DO APP
        msg = f"🏛 *PRIVATE BANK - CONSOLIDADO*\n"
        msg += f"📅 _{data_hora}_\n\n"
        ...
        msg += f"\n💰 *PATRIMÔNIO TOTAL:* \n`{formatar_br(total)}`"
        msg += f"\n\n💸 *PROVENTOS ESTIMADOS (MÊS):*\n`{formatar_br(dividendos_estimados)}`"

        # 3. BUSCAR NOTÍCIAS EM TEMPO REAL
        try:
            feed = yf.Ticker("^BVSP").news[:4]
            if feed:
                msg += "\n\n📰 *NOTÍCIAS DO MERCADO (AGORA):*"
                for n in feed:
                    msg += f"\n• [{n['title'][:55]}...]({n['link']})"
        except: pass
        ...
        print(f"✅ Relatório enviado com sucesso! Total: {total}")
